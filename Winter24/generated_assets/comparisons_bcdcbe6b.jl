...
# ╔═╡ 43fcd269-3b33-4bd6-9dab-926851b10820
using CategoricalArrays

# ╔═╡ 507537e3-897d-4e85-8299-5d5a433efd31
using DataFrames

# ╔═╡ 6f8da0c9-3676-43fa-a76a-13102d17b4ca
using Dates: year

# ╔═╡ 62e0c239-f7a3-4956-947a-f32517da999b
...
# ╔═╡ 1891eafd-507c-41b3-995d-121d355ae101
import CSV, HTTP
...
debt_fig = let d = agg_df
	...
	for (i,dbt) in enumerate(dbt_var)
		...
		for (j, fractionof) in enumerate([var, :hwdeb2inc])
			lines!(axs[j], d.year, d[!,var]/d[i80,fractionof], label = string(dbt)
			)
		end
	end
...
d2y_fig = let
	...
	for (i,d) in enumerate(groupby(df3, :three_groups))
			i80 = findfirst(d.year .== 1980)
			lines!(ax, d.year, d.mort2inc ./ d.mort2inc[i80], 
				label = only(unique(d.three_groups))
			)
	end
...
ineq_fig = let
	...
	for (i,d) in enumerate(groupby(df3, :three_groups))
			i80 = findfirst(d.year .== 1980)
			lines!(ax, d.year, d[:,r_inc] ./ d[i80,r_inc], label = only(unique(d.three_groups)))
	end
	
	vlines!(ax, [1980, 2007], color = :gray, linestyle = :dash)
	Legend(fig[1,2], ax)
	fig
end
...
manifest_format = "2.0"
project_hash = "1d50cb1968564d0ae549bbc3599f254e4133ce55"
